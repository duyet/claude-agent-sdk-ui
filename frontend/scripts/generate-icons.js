...
async function generateIcons() {
  for (const { size, name } of sizes) {
    await sharp(Buffer.from(svgContent)).resize(size, size).png().toFile(path.join(appDir, name))
    console.log(`Generated ${name}`)
  }

  console.log("All icons generated successfully!")
  console.log(
    "Note: favicon.ico not generated - using PNG/SVG instead (modern browsers support them)",
  )
}
